...
async def delete_all_user_chats(request: Request):
...
async def create_new_chat(form_data: ChatForm):
    try:
...
async def delete_chat_by_id(request: Request, id: str):
...
async def clone_chat_by_id(id: str):
    chat = Chats.get_chat_by_id_and_user_id(id, user.id)
    if chat:
...
async def archive_chat_by_id(id: str):
    chat = Chats.get_chat_by_id_and_user_id(id, user.id)
    if chat:
...
async def share_chat_by_id(id: str):
    chat = Chats.get_chat_by_id_and_user_id(id, user.id)
    if chat:
        if chat.share_id:
...
async def delete_shared_chat_by_id(id: str):
    chat = Chats.get_chat_by_id_and_user_id(id, user.id)
    if chat:
...
async def get_chat_tags_by_id(id: str):
...
async def add_chat_tag_by_id(
    id: str, form_data: ChatIdTagForm
):
    tags = Tags.get_tags_by_chat_id_and_user_id(id, user.id)
...
async def delete_all_chat_tags_by_id(id: str):
